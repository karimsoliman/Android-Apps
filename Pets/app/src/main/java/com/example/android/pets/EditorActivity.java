...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private void savePet(){
        ...
        String interedWeight = mWeightEditText.getText().toString().trim();

        if (petUri == null &&
                TextUtils.isEmpty(petName) && TextUtils.isEmpty(petBreed) &&
                TextUtils.isEmpty(interedWeight) && mGender == PetEntry.GENDER_UNKNOWN) {return;}

        ContentValues newPet = new ContentValues();
        newPet.put(PetEntry.COLUMN_PET_NAME, petName);
        newPet.put(PetEntry.COLUMN_PET_BREED, petBreed);
        int weight = 0;
        if (!TextUtils.isEmpty(interedWeight)) {
            weight = Integer.parseInt(interedWeight);
        }
        newPet.put(PetEntry.COLUMN_PET_WEIGHT, weight);
        newPet.put(PetEntry.COLUMN_PET_GENDER, mGender);

        if(petUri == null){
            Uri insertUri = getContentResolver().insert(PetEntry.CONTENT_URI, newPet);
            if (insertUri == null) {
            ...
            int updatedRows = getContentResolver().update(petUri, newPet, null, null);
            if (updatedRows == 0){
                Toast.makeText(this, getString(R.string.error_saving_pet), Toast.LENGTH_SHORT).show();
            } else {
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (cursor.moveToFirst()) {
            int petNameIndex = cursor.getColumnIndex(PetEntry.COLUMN_PET_NAME);
            int petBreedIndex = cursor.getColumnIndex(PetEntry.COLUMN_PET_BREED);
            int petGenderIndex = cursor.getColumnIndex(PetEntry.COLUMN_PET_GENDER);
            int petWeightIndex = cursor.getColumnIndex(PetEntry.COLUMN_PET_WEIGHT);

            String petName = cursor.getString(petNameIndex);
            String petBreed = cursor.getString(petBreedIndex);
            int petGender = cursor.getInt(petGenderIndex);
            int petWeight = cursor.getInt(petWeightIndex);
